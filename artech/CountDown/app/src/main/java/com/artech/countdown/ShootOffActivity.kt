...
class ShootOffActivity : AppCompatActivity() {

    val mDbManager: DBManager = DBManager(this, "count.db", null, 1)

    var time: Int = 0
    var waiting: Int = 10//9//13//11
    var max: Int = 241
    var round: Int = 0
    var withdraw: Int = 0
    var roundFlag: Int = 0
...
//                timer.schedule(timerTask { doSomething() }, 1000, 1000)
                timer.schedule(timerTask { doSomething() }, 10, 1000)
            }
        }
        ...
        pause_button.setOnClickListener {
            if(mPlayBoolean) {
                if (mPlayFlag == 0) {
                    waiting = 0
                }else if (mPlayFlag == 1 || mPlayFlag == 2) {
                    time = max-1
                } else if (mPlayFlag == 3) {
                    time = withdraw-1
                }
...
//                    if(round == 1) {
//                        Log.d("CountActivity", "test round : " + round + " / last : " + last + "/ time : " + time)
//                        if ((time < last-6) && (time%2==0)) {
//                            if (mSpeaker == 0)
//                                sound.play(soundId_1, 1.0f, 1.0f, 0, 0, 1.0f)
//                            else if (mSpeaker == 1)
//                                sound.play(soundId_2, 1.0f, 1.0f, 0, 0, 1.0f)
