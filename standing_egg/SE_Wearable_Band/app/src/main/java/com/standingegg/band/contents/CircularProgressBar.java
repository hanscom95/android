...
public class CircularProgressBar extends View {
	...
	public CircularProgressBar(final Context context, final AttributeSet attrs, final int defStyle) {
		...
		final TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.CircularProgressBar, defStyle,
				0);
		if (attributes != null) {
			try {
				setProgressColor(attributes.getColor(R.styleable.CircularProgressBar_progress_color, Color.RED));
				setProgressBackgroundColor(
						attributes.getColor(R.styleable.CircularProgressBar_progress_background_color, Color.WHITE));
				...
				setWheelSize((int) attributes.getDimension(R.styleable.CircularProgressBar_stroke_width, 0));

				setThumbEnabled(attributes.getBoolean(R.styleable.CircularProgressBar_thumb_visible, false));
				setMarkerEnabled(attributes.getBoolean(R.styleable.CircularProgressBar_marker_visible, true));
		...
		mThumbRadius = mCircleStrokeWidth;// * 2; // * 2;
	...
	protected void onDraw(final Canvas canvas) {
		...
		// Canvas.drawArc(RectF oval, float startAngle, float sweepAngle,
		...
		if (mIsMarkerEnabled) {
			...
			canvas.drawLine((float) (mThumbPosX + mThumbRadius / 2 * 1.3), mThumbPosY,
					(float) (mThumbPosX - mThumbRadius / 2 * 1.3), mThumbPosY, mMarkerColorPaint);

			// canvas.draw
			// void android.graphics.Canvas.drawLine(float startX, float startY,
...
//			canvas.drawArc(mSquareRect, 45, 90, true, mThumbColorPaint);
			canvas.drawCircle(mThumbPosX, mThumbPosY, mThumbRadius / 16, mThumbColorPaint);
			canvas.restore();
		}
	}
	...
	public void setWheelSize(final int dimension) {
		mCircleStrokeWidth = dimension * 2;
	...
	public void setDrawableImg(final int dimension) {
	...
	private void updateBackgroundColor() {
		...
		mBackgroundColorPaint.setShadowLayer(0.05f, 0.5f, 0.5f, Color.WHITE);
	...
	private void updateProgressColor() {
		...
		mProgressColorPaint.setShadowLayer(mRadius + 20, mHorizontalInset, mVerticalInset, Color.WHITE);
