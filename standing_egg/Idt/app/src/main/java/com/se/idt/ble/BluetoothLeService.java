...
public class BluetoothLeService extends Service {
    ...
    public static final int MOTION_KNEELING_ONE_ARM_ROW = 16;
    public static final int MOTION_LATERAL_RAISE = 17;
    public static final int MOTION_OTHER = 18;
    public static final int MOTION_WALKING_POCKET = 19;
    public static final int MOTION_WALKING_HAND = 20;
    public static final int MOTION_WALKING_READING = 21;
    public static final int MOTION_WALKING_BAG = 22;
    public static final int MOTION_INVALID = 99;
...
//            Log.d(TAG, "# onCharacteristicChanged " + characteristic.toString());
    ...
    protected void writeNextDescriptor() {
        if (iterDescriptor.hasNext()) {
            ...
            try {
//                if (lastDescriptor.getCharacteristic() == mGattCharacteristics.get(1).get(1)) {
//                    lastDescriptor.setValue(isSetProperty(mGattCharacteristics.get(1).get(1)
    ...
    public void displayGattServices(List<BluetoothGattService> gattServices) {
        ...
        if (mGattCharacteristics != null) {
            try {
                BluetoothGattCharacteristic characteristic;
//                characteristic = mGattCharacteristics.get(1).get(1);
    ...
    //            Log.d("BluetoothLeService", "mGattCharacteristics uuid : " + characteristic.getUuid());
                /*if(mGattCharacteristics.size() < 3) {
                    Log.d("BluetoothLeService", "여기들옴 1");
                    characteristic = mGattCharacteristics.get(1).get(1);
                }else {
    ...
    public void bleWrite(byte[] data) {
        if(data.length > 20) {
            ...
            transaction.begin();
            transaction.setMessage(sByte);
            transaction.sendTransaction();

            byte[] eByte = new byte[data.length-20];
            for(int j = 0; j < eByte.length; j++) {
                eByte[j] = data[20+j];
            }
            transaction.begin();
            transaction.setMessage(eByte);
            transaction.sendTransaction();
        }else {
            TransactionBuilder.Transaction transaction = mTransactionBuilder.makeTransaction();
            transaction.begin();
    ...
    public boolean write(BluetoothGattCharacteristic chr, byte[] data) {
        ...
        try {
//            writableChar = mGattCharacteristics.get(1).get(0);
